...
monthly_sales_counts = sales_df.groupby(['Year', 'Month', 'Store']).size().groupby(['Year', 'Month']).count().reset_index(name='Liczba sklepów')
...
# Wyniki
print("Liczba sklepów danego typu (A, B, C):\n", store_types_counts.to_string(index=False, header=False))
print("\nStatystyki dla rozmiaru sklepów:\n", size_statistics.to_string(index=False, header=False))
